...
def convert_state_dict_to_peft(state_dict, original_type=None, **kwargs):
    r"""
    The method only supports the conversion from diffusers new to PEFT.
    ...
    """
    if any("lora_linear_layer" in k for k in state_dict.keys()):
...
def convert_state_dict_to_diffusers(state_dict, original_type=None, **kwargs):
    r"""
    The method only supports the conversion from PEFT to diffusers new.
